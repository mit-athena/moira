@Comment[
	...
	$Author: wesommer $
	$Header: /afs/.athena.mit.edu/astaff/project/moiradev/repository/doc/tech-plan/server.mss,v 1.2 1987-06-23 15:53:37 wesommer Exp $
...
server, using a remote procedure call protocol built on top of
GDB@cite(GDB).  The SMS server runs as one or more UNIX processes on
the SMS database machine.  It listens for TCP/IP connections on a well
known service port (T.B.D.), and processes remote procedure call
requests on each connection it accepts.
...
takes to start up two processes: the user information daemon, and the
Ingres back end subprocess which it uses to access the database.  As
starting up a backend process is a rather heavyweight operation, the
SMS server will do this only once, at the start up time of the daemon.
...
access caching will be worked into the server for performance reasons.
