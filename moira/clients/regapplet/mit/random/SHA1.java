...
//...........................................................................
    ...
    private static void debug(String s) {if (DEBUG) { err.println("SHA-1: " + s); }}
...
//...........................................................................
  ...
  //    }
     ...
     *
     * @param in      Array of bytes to be added
     */
    public void Update(byte[] in)
    {
        byte b[] = java_digest(in, in.length);
    }
...
if (DEBUG) debug("A="+A+" B="+B+" C="+C+" D="+D+" E="+E);

        int W[] = w;
        for (int i=0; i<16; i++)
        {
            W[i] = X[i];
if (DEBUG) debug("       " + i + "=" + X[i]);
        }
        for (int i=16; i<80; i++)
        {
        ...
        digest[3] += D;
        digest[4] += E;
if (DEBUG) debug("d1="+digest[0]+" d1="+digest[1]+" d2="+digest[2]+" d3="+digest[3]+" d4="+digest[4]);
    }
...
throws Exception
{
	...
	byte [] hash1 = hash.Digest(text);
	debug("Computed: " + Hex.toString(hash1));
	debug("Known:    " + Hex.toString(hashs[i]));
        if (notEquals(hash1, hashs[i]))
            throw new MITCipherException("hash #"+ i +" failed");
    }       
}
